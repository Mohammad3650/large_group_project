...
vi.mock('../SubscriptionForm.jsx', () => ({
    default: ({ onImport }) => (
        <button
            onClick={() =>
                onImport({ name: 'Test', source_url: 'http://test.com' })
            }
        >
            Import
        </button>
...
const in3DaysStr = formatDate(
    new Date(today.getFullYear(), today.getMonth(), today.getDate() + 3)
);
const in10DaysStr = formatDate(
...
const defaultTaskGroups = {
    overdueTasks: [mockBlocks[0]],
    setOverdueTasks: vi.fn(),
    todayTasks: [mockBlocks[1]],
    setTodayTasks: vi.fn(),
    tomorrowTasks: [mockBlocks[2]],
    setTomorrowTasks: vi.fn(),
    weekTasks: [mockBlocks[3]],
    setWeekTasks: vi.fn(),
    beyondWeekTasks: [mockBlocks[4]],
...
describe('Tests for Dashboard', () => {
    ...
    it('renders an error message when fetching subscriptions fails', async () => {
        getCalendarSubscriptionsModule.default.mockRejectedValue(
            new Error('Network error')
        );
        renderDashboard();
        await waitFor(() =>
            expect(
                screen.getByText('Failed to load calendar subscriptions')
            ).toBeInTheDocument()
        );
    });
    ...
    it('shows the no-tasks message when there are no tasks', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText('🎉 Congrats, you have no tasks!')
            ).toBeInTheDocument()
        );
    });
    ...
    it('does not show the no-tasks message when there are tasks', async () => {
        ...
        expect(
            screen.queryByText('🎉 Congrats, you have no tasks!')
    ...
    it('renders the After Next 7 Days task group with the correct task', async () => {
        renderDashboard();
        await waitFor(() =>
            expect(screen.getByText('After Next 7 Days')).toBeInTheDocument()
        );
    ...
    it('renders an error when blocksError is set', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText('Failed to load time blocks')
            ).toBeInTheDocument()
        );
    });
    ...
    it('renders an error when import fails', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText('Failed to import timetable')
            ).toBeInTheDocument()
        );
    });
    ...
    it('refreshes a subscription when refresh is clicked', async () => {
        ...
        expect(refreshCalendarSubscriptionModule.default).toHaveBeenCalledWith(
            1
        );
    });
    ...
    it('renders an error when refresh fails', async () => {
        refreshCalendarSubscriptionModule.default.mockRejectedValue(
            new Error('Network error')
        );
        renderDashboard();
        await waitFor(() =>
        ...
        await waitFor(() =>
            expect(
                screen.getByText('Failed to refresh timetable subscription')
            ).toBeInTheDocument()
        );
    });
    ...
    it('deletes a subscription when delete is clicked', async () => {
        ...
        expect(deleteCalendarSubscriptionModule.default).toHaveBeenCalledWith(
            1
        );
    });

    it('renders an error when deletion fails', async () => {
        deleteCalendarSubscriptionModule.default.mockRejectedValue(
            new Error('Network error')
        );
        renderDashboard();
        await waitFor(() =>
        ...
        await waitFor(() =>
            expect(
                screen.getByText('Failed to delete timetable subscription')
            ).toBeInTheDocument()
        );
    });
});
