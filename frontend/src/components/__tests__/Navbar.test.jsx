...
describe("Navbar", () => {
    ...
    describe("when the user is not logged in", () => {
        ...
        it("does not render the Dashboard or Calendar links", async () => {
            renderNavbar();
            await waitFor(() => expect(authToken.isTokenValid).toHaveBeenCalled());
            expect(screen.queryByText("Dashboard")).not.toBeInTheDocument();
            expect(screen.queryByText("Calendar")).not.toBeInTheDocument();
        });

        it("renders the 'Built for Students' tagline", async () => {
            renderNavbar();
            await waitFor(() => expect(authToken.isTokenValid).toHaveBeenCalled());
            expect(screen.getByText("Built for Students")).toBeInTheDocument();
        });

        it("does not render the user icon", async () => {
            renderNavbar();
            await waitFor(() => expect(authToken.isTokenValid).toHaveBeenCalled());
    ...
    describe("when the user is logged in", () => {
        ...
        it("closes the dropdown when the user icon is clicked a second time", async () => {
            ...
            fireEvent.click(screen.getByAltText("User")); // open
            fireEvent.click(screen.getByAltText("User")); // close
