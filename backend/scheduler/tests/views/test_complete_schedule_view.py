...
class CompleteScheduleViewTest(APITestCase):
    def setUp(self):
        """Sets up an authenticated user, a second user, and a sample TimeBlock for completion tests."""
    ...
    def test_complete_time_block_success(self):
        """Tests that a time block is successfully marked as completed."""
        response = self.client.patch(self.url)
        self.assertEqual(response.status_code, 200)
        self.block.refresh_from_db()
        self.assertIsNotNone(self.block.completed_at)

    def test_complete_time_block_clears_pinned(self):
        """Tests that completing a pinned time block clears the pinned flag and pinned_at."""
        self.block.pinned = True
        self.block.pinned_at = timezone.now()
        self.block.save()
    ...
    def test_complete_other_users_block_returns_404(self):
        """Tests that a user cannot complete a time block belonging to another user."""
